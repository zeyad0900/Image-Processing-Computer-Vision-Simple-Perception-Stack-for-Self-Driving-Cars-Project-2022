...
def find_cars(img, ystart, ystop, scale, svc, X_scaler, orient, pix_per_cell, cell_per_block, spatial_size, hist_bins, cspace='YCrCb', hog_channel='ALL')
    ...
    img_tosearch = img[ystartystop,xstart,]
    ...
    # nfeat_per_block = orientcell_per_block2
    ...
    for xb in range(nxsteps)
        for yb in range(nysteps)
            ypos = ybcells_per_step
            xpos = xbcells_per_step
            ...
            hog_feat1 = hog1[yposypos+nblocks_per_window, xposxpos+nblocks_per_window].ravel() 
            hog_feat2 = hog2[yposypos+nblocks_per_window, xposxpos+nblocks_per_window].ravel() 
            hog_feat3 = hog3[yposypos+nblocks_per_window, xposxpos+nblocks_per_window].ravel() 
            ...
            xleft = xpospix_per_cell
            ytop = ypospix_per_cell

            # Extract the image patch
            subimg = cv2.resize(ctrans_tosearch[ytopytop+window, xleftxleft+window], (64,64))
            ...
            if test_prediction == 1
                xbox_left = np.int(xleftscale)
                ytop_draw = np.int(ytopscale)
                win_draw = np.int(windowscale)
...
def draw_labeled_bboxes(img, labels)
    ...
    for car_number in range(1, labels[1]+1)
        ...
        # Define a bounding box based on minmax x and y
...
def trainSVC(filename = 'svc.pickle', colorspace = 'YCrCb', orient = 9, pix_per_cell = 8, cell_per_block = 2, spatial_size = (32, 32), hist_bins = 32, hog_channel = 'ALL', sample_size = 1000)
    ...
    # car_images = glob.glob('train_datavehicles_smallset.jpeg')
    car_images = glob.glob('train_datavehicles.png')
    cars = []
    for image in car_images
        cars.append(image)

    # notcar_images = glob.glob('train_datanot-vehicles_smallset.jpeg')
    notcar_images = glob.glob('train_datanon-vehicles.png')
    notcars = []
    ...
    if sample_size != None
        cars = cars[0sample_size]
        notcars = notcars[0sample_size]
        # notcars = notcars[0sample_size + 500]
